...
from app.create_group import create_group_main_page,get_user_created_groups,search_group,get_group_details,edit_group_details,user_request_for_group,deactivate_group,activate_group,get_group_dashboard,remove_or_reject_user_from_group,accept_user_join_request,send_message_to_group_members_api,edit_group_comment,remove_group_comment
...
urlpatterns = [
    ...
    url(r'^(?P<group_id>\d+)/comment/(?P<comment_id>\d+)/edit/$',edit_group_comment,name = 'edit_group_comment'),
    url(r'^(?P<group_id>\d+)/comment/(?P<comment_id>\d+)/remove/$',remove_group_comment,name = 'remove_group_comment'),
]
